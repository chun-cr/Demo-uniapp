...
class RecordManager {
    ...
    constructor(id = 'default') {
        ...
        // 音量计算相关
        this.volumeHistory = []      // 历史RMS值
        this.maxHistorySize = 100    // 最大历史记录数
        this.silenceThreshold = 70   // 静音阈值
        this.lastVolume = 0          // 上次音量（用于平滑）
    ...
    */
    ...
    calculateVolume(frameData) {
        ...
        const rms = Math.sqrt(sumSquared / frameData.length - mean * mean);
        ...
        if (this.volumeHistory.length >= 20) {
            ...
            const baseline = sortedHistory[Math.floor(sortedHistory.length * 0.1)]; // 取10%分位数
            this.silenceThreshold = baseline * 1.5; // 比基线高50%
        }
        
        // 4. 计算音量（使用对数刻度，符合人耳感知）
        let volume = 0;
        if (rms > this.silenceThreshold) {
            // 使用对数刻度（分贝）更符合人耳
            const db = 20 * Math.log10((rms - this.silenceThreshold) / 50 + 1e-6);
            volume = Math.min(Math.max(db + 50, 0), 100);
        }
        
        // 5. 低通滤波，使音量变化更平滑
        volume = this.lowPassFilter(volume);
    ...
    lowPassFilter(newValue) {
        ...
        // 简单的一阶低通滤波器
        const smoothed = this.lastVolume * 0.7 + newValue * 0.3;
