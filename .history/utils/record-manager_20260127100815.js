...
class RecordManager {
    ...
    */
    ...
    calculateVolume(frameData) {
        ...
        // 计算标准差作为 RMS，这比单纯的能量更稳健
        const rms = Math.sqrt(Math.max(sumSquared / length - mean * mean, 0));
        ...
        if (this.volumeHistory.length >= 20) {
            // 取历史记录的 10% 分位数作为当前环境的底噪参考
            const sortedHistory = [...this.volumeHistory].sort((a, b) => a - b);
            const baseline = sortedHistory[Math.floor(sortedHistory.length * 0.1)];
            // 阈值设为底噪的 1.5 倍
        ...
        if (rms > this.silenceThreshold) {
            // 使用对数刻度（类分贝）转换，更符合听感
            // 这里的 50 是一个范围系数，根据效果可以微调
            const db = 20 * Math.log10((rms - this.silenceThreshold) / 50 + 1);
            // 将 dB 映射到 0-100 范围
            volume = Math.round(Math.min(Math.max(db * 10, 0), 100));
        }
        ...
        if (this.lastVolume === 0) {
            ...
            // 70% 保留旧值，30% 采用新值，防止跳变
            this.lastVolume = this.lastVolume * 0.7 + volume * 0.3;
        }

        return Math.round(this.lastVolume);
    }
