this["web-track-browser"] = (function () {
  ...
  class WebTracker {
    constructor(opt) {
      ...
      this.ccController = new Concurrent((_c = opt.maxReqCount) != null ? _c : 10);
    ...
    platformSend() {
      if (this.opt.sendPayload && typeof this.opt.sendPayload === "function") {
        ...
        this.ccController.append(() => this.opt.sendPayload(this.url, payloadStr));
      }
    }
    transString(obj) {
      let newObj = {};
            ...
            if (httpRequest_.readyState === 4) {
              if (httpRequest_.status === 200) {
                ...
                if (window && window.console && typeof window.console.error === "function") {
                  ...
                  console.error("Failed logdata:", url);
                }
              }
              resolve();
            }
  ...
  class WebTrackerBrowser extends WebTracker {
    constructor(opt) {
      const superOpt = Object.assign({}, opt, {
        ...
        sendPayload: (url, payload) => __async(this, null, function* () {
          send(url, payload);
        })
      });
      super(superOpt);
    ...
    useStsPlugin(stsPlugin) {
      this.getOpt().sendPayload = (url, payload) => __async(this, null, function* () {
        yield sendPutlog(url, payload, stsPlugin);
      });
